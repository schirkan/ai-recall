...
- **API-Call**: `SpeechConfig.FromSubscription(key, region)` →
  `AudioConfig.FromWavFileInput(path)` →
...
- **Request-Body**: Multipart mit `mic.wav` + `loopback.wav` (zwei separate Calls oder zusammengeführt; v0.3: zwei separate Calls, später Merge-Logik)
